...
import type { BackendDocument, ChatMessage, ChatSession, CourseListItem, RagCitation } from '@/types'
...
const suggestions = ['总结当前资料的核心内容', '列出三个复习重点', '根据资料解释一个关键概念']
...
<template>
  <div class="chat-page">
    <PageHeader title="智能问答" eyebrow="RAG ASSISTANT" description="选择真实课程和已就绪资料，回答与引用会持久化到会话中。">
    ...
    <section v-else class="chat-shell content-card">
      ...
      <main class="conversation">
        <div class="conversation-head"><div><span class="ai-avatar">✦</span><p><b>StudyPilot AI</b><small><i></i>{{ selectedCourse?.name }}</small></p></div><span class="model-pill">RAG · 严格引用</span></div>
        ...
        <template v-else>
          <div ref="messageArea" v-loading="messagesLoading" class="messages">
            ...
            <el-empty v-if="!messagesLoading && !messages.length && !messagesError" description="新对话尚无消息，可以从下方开始提问" />
            <div v-for="message in messages" :key="message.id" class="message" :class="message.role">
              <span class="message-avatar"><el-icon v-if="message.role === 'user'"><User /></el-icon><template v-else>✦</template></span>
              <div class="message-block">
                <div class="message-meta"><b>{{ message.role === 'user' ? '你' : 'StudyPilot AI' }}</b><span>{{ formatTime(message.created_at) }}</span><em v-if="message.role === 'assistant' && message.sufficient_evidence === false">证据不足</em></div>
                <div class="bubble"><p v-for="(paragraph,index) in message.content.split('\n\n')" :key="index">{{ paragraph }}</p></div>
                <div v-if="message.citations.length" class="answer-tools"><button @click="showSources(message)"><el-icon><Document /></el-icon>{{ message.citations.length }} 条真实引用</button></div>
              </div>
            </div>
            <div v-if="sending" class="message assistant"><span class="message-avatar">✦</span><div class="message-block"><div class="message-meta"><b>StudyPilot AI</b><span>正在检索课程资料</span></div><div class="typing"><i></i><i></i><i></i><span>正在读取并匹配已就绪资料…</span></div><small class="pending-question">问题：{{ pendingQuestion }}</small></div></div>
          </div>
