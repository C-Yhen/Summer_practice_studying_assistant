...
def test_recommendations_only_include_incomplete_tasks_from_active_plan_version(
    ...
    with client.app.state.database.session_factory() as db:
        ...
        point = KnowledgePoint(course_id=course_a, name="Active task topic", description="Concrete source-backed task topic")
        db.add(point)
        db.flush()
        active_task = _task(active.id, user_id, course_a, "same title", point_id=point.id)
        completed = _task(active.id, user_id, course_a, "completed", "completed", point_id=point.id)
        old_task = _task(old.id, user_id, course_a, "same title", point_id=point.id)
        candidate_task = _task(candidate.id, user_id, course_a, "candidate", point_id=point.id)
